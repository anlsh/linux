...
::
  ...
  #define KVM_MEM_EXIT_ON_MISSING  (1UL << 2)
...
3.  KVM_MEM_EXIT_ON_MISSING: see KVM_CAP_USERFAULT_ON_MISSING for details.
...
When KVM_RUN returns an error with errno=EFAULT, an exit reason of
KVM_EXIT_MEMORY_FAULT indicates that the kvm_run.memory_fault field is valid.
This capability is only partially implemented in that not all EFAULTs from
KVM_RUN may be annotated in this way: these "bare" EFAULTs should be considered
bugs and reported to the maintainers.
...
KVM_MEM_EXIT_ON_MISSING on memslots (via KVM_SET_USER_MEMORY_REGION). Said
