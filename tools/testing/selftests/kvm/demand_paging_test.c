 ...
 * vcpu_worker in the case of a GOOGLE_KVM_EXIT_EFAULT_INFO.
 */
...
{
	int r;
	void *hva = addr_gpa2hva(perf_test_args.vm, gpa);
...
static void ready_page(uint64_t gpa)
{
	 ...
	 * MADV_POPULATE_WRITE.
	 */
	if ((gpa < perf_test_args.gpa)
		|| (gpa >= perf_test_args.gpa + perf_test_args.size)) {
		madv_write_or_err(gpa);
	} else {
		if (uffd_delay)
			usleep(uffd_delay);

		uffd = uffd_descs[(gpa - perf_test_args.gpa) / uffd_region_size]->uffd;

		r = handle_uffd_page_request(uffd_mode, uffd,
					     (uint64_t) addr_gpa2hva(perf_test_args.vm, gpa), true);
...
static void vcpu_worker(struct perf_test_vcpu_args *vcpu_args)
{
	...
	while (true) {
		...
		TEST_ASSERT(ret == 0
			    || (errno == EFAULT
				&& run->exit_reason == GOOGLE_KVM_EXIT_EFAULT_INFO),
			    "vcpu_run failed: %d\n", ret);
		...
		if (ret != 0 && get_ucall(vcpu, NULL) != UCALL_SYNC) {
			...
			if (run->exit_reason == GOOGLE_KVM_EXIT_EFAULT_INFO) {
				ready_page(run->efault_info.gpa);
				continue;
			}
...
static void run_test(enum vm_guest_mode mode, void *arg)
{
	...
	if (uffd_memfault_exits) {
		TEST_ASSERT(kvm_has_cap(GOOGLE_KVM_CAP_MEMORY_FAULT_INFO) > 0,
					"KVM does not have GOOGLE_KVM_CAP_MEMORY_FAULT_INFO");
		TEST_ASSERT(kvm_has_cap(GOOGLE_KVM_CAP_NOWAIT_ON_FAULT) > 0,
					"KVM does not have GOOGLE_KVM_CAP_NOWAIT_ON_FAULT");
		slot_flags = GOOGLE_KVM_MEM_NOWAIT_ON_FAULT;
	}
	...
	vcpu_paging_rate =
		perf_test_args.vcpu_args[0].pages
...
static void help(char *name)
{
	...
	printf("usage: %s [-h] [-m vm_mode] [-u uffd_mode] [-a]\n"
	       "          [-d uffd_delay_usec] [-r readers_per_uffd] [-w]\n"
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "hoawm:u:d:b:s:v:r:")) != -1) {
		switch (opt) {
		case 'm':
